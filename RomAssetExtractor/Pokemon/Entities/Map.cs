...
namespace RomAssetExtractor.Pokemon.Entities
{
    ...
    public class Map : MapModel
    {
        ...
        public MetatileLayout Metatiles { get; private set; }

        /// <summary>Parsed map events: NPCs, warps, triggers, signs.</summary>
        public MapEvents Events { get; internal set; }
        ...
        public static Map ReadMap(PokemonRomReader reader, int bankId, int mapId)
        {
            ...
            // Read map events (NPCs, warps, triggers, signs)
            map.Events = MapEvents.ReadEvents(reader, map.Header.Events);
